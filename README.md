...
├── frontend/                # React 前端
│   ├── src/
│   │   ├── components/      # 公共组件（Layout、Sidebar）
...
| `/api/schedules` | GET/POST | 日程列表 / 创建日程 | 是 |
| `/api/schedules/date/{date}` | GET | 按日期获取日程 | 是 |
| `/api/diaries` | GET/POST | 日记列表 / 创建日记 | 是 |
| `/api/diaries/date/{date}` | GET | 按日期获取日记 | 是 |
| `/api/analysis/daily` | GET | 日情绪分析 | 是 |
| `/api/analysis/weekly` | GET | 周情绪分析 | 是 |
| `/api/analysis/monthly` | GET | 月情绪分析 | 是 |
| `/api/settings` | GET/PUT | 获取 / 更新用户设置 | 是 |

## 关键设计

- **用户数据隔离**：所有数据查询均通过 JWT 中的用户 ID 过滤，确保多租户安全
- **RESTful API**：统一的资源导向 URL 设计，使用标准 HTTP 方法
- **统一响应格式**：后端返回 `ApiResponse<T>` 标准结构（code / message / data）
- **密码安全**：使用 BCrypt 强哈希存储用户密码
